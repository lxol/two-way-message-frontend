...
class EnquiryControllerSpec extends ControllerSpecBase with MockAuthConnector with I18nSupport {
  ...
  "calling onPageLoad() for P800" should {
    ...
    "return 200 (OK) when presented with a valid Nino (HMRC-NI) enrolment from auth-client" in {
      ...
      when(mockPreferencesConnector.getPreferredEmail(any[String])(any[HeaderCarrier])).thenReturn(Future.successful("preferredEmail@test.com"))
      mockAuthorise(AuthProviders(GovernmentGateway), Retrievals.nino)(Future.successful(Some(nino.value)))
    ...
    "return 401 (UNAUTHORIZED) when auth-client returns 401" in {
      when(mockPreferencesConnector.getPreferredEmail(any[String])(any[HeaderCarrier])).thenReturn(Future.successful("preferredEmail@test.com"))
      mockAuthorise(AuthProviders(GovernmentGateway), Retrievals.nino)(Future.failed(new UnsupportedAffinityGroup("")))
      val result = call(controller.onPageLoad("p800-overpayment"), fakeRequest)
  ...
  "calling onPageLoad() for ct-general" should {
    ...
    "return 200 (OK) when presented with a missing Nino identifier and an enrolment other than HMRC-NI from auth-client" in {
      ...
      mockAuthorise(AuthProviders(GovernmentGateway), Retrievals.nino)(Future.successful(None))
      val result = call(controller.onPageLoad("epaye-general"), fakeRequest)
      verify(mockPreferencesConnector, never()).getPreferredEmail(any[String])(any[HeaderCarrier])
      status(result) shouldBe Status.OK
      val document = Jsoup.parse(contentAsString(result))
      document.getElementsByClass("heading-large").text().contains("Send your message") shouldBe true
    }
